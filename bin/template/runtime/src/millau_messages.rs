...
/// Initial value of `MillauToTemplateConversionRate` parameter.
pub const INITIAL_MILLAU_TO_TEMPLATE_CONVERSION_RATE: FixedU128 = FixedU128::from_inner(FixedU128::DIV);

parameter_types! {
	/// Millau to Template conversion rate. Initially we treat both tokens as equal.
	pub storage MillauToTemplateConversionRate: FixedU128 = INITIAL_MILLAU_TO_TEMPLATE_CONVERSION_RATE;
}
...
/// Message verifier for Template -> Millau messages.
...
/// Encoded Template Call as it comes from Millau.
pub type FromMillauEncodedCall = messages::target::FromBridgedChainEncodedMessageCall<WithMillauMessageBridge>;

/// Call-dispatch based message dispatch for Millau -> Template messages.
...
/// Messages proof for Millau -> Template messages.
pub type FromMillauMessagesProof = messages::target::FromBridgedChainMessagesProof<bp_millau::Hash>;

/// Messages delivery proof for Template -> Millau messages.
...
impl MessageBridge for WithMillauMessageBridge {
	...
	type ThisChain = Template;
	type BridgedChain = Millau;

	fn bridged_balance_to_this_balance(bridged_balance: bp_millau::Balance) -> bp_template::Balance {
		bp_template::Balance::try_from(MillauToTemplateConversionRate::get().saturating_mul_int(bridged_balance))
			.unwrap_or(bp_template::Balance::MAX)
	}
}
...
impl messages::ThisChainWithMessages for Template {
	...
	fn estimate_delivery_confirmation_transaction() -> MessageTransaction<Weight> {
		let inbound_data_size = InboundLaneData::<bp_template::AccountId>::encoded_size_hint(
			bp_template::MAXIMAL_ENCODED_ACCOUNT_ID_SIZE,
			1,
		)
		.unwrap_or(u32::MAX);
...
impl TargetHeaderChain<ToMillauMessagePayload, bp_millau::AccountId> for Millau {
	...
	) -> Result<(LaneId, InboundLaneData<bp_template::AccountId>), Self::Error> {
...
/// Template -> Millau message lane pallet parameters.
#[derive(RuntimeDebug, Clone, Encode, Decode, PartialEq, Eq)]
pub enum TemplateToMillauMessagesParameter {
	/// The conversion formula we use is: `TemplateTokens = MillauTokens * conversion_rate`.
	MillauToTemplateConversionRate(FixedU128),
}

impl MessagesParameter for TemplateToMillauMessagesParameter {
	fn save(&self) {
		match *self {
			TemplateToMillauMessagesParameter::MillauToTemplateConversionRate(ref conversion_rate) => {
				MillauToTemplateConversionRate::set(conversion_rate)
			}
		}
	}
}
